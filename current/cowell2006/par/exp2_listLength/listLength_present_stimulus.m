function [W, initial_selec, p, pktot] = listLength_present_stimulus(stim, W, p, trial,pktot)
    ...
    for grid = 1:p.nGrids(layer),
        ...
        for cycle=1:p.numEncodingCycles
            ...
            [f, selectivity, p, act_peak, act_total] = ...
                listLength_calc_selectivity(win_row, win_col, dist_mat, p, p.numInputDims(layer));
...
%                 initial_weights(layer,:,:,1:p.numInputDims(layer),grid) = weights;
        ...
        % completed
        [win_row, win_col, dist_mat] = findWinningNode(weights, input_mat, p.numInputDims(layer));
        [~, selectivity, p, act_peak, act_total] = listLength_calc_selectivity(win_row, win_col, dist_mat, p, p.numInputDims(layer));
