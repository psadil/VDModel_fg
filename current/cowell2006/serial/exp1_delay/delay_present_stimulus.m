function [W, initial_selec, p, pktot] = delay_present_stimulus(stim, W, p, trial,pktot)
...
for layer=1:p.layer
    ...
    for grid = 1:p.nGrids(layer),
        ...
        for cycle=1:nEncodCycles
            ...
            %--------------------------------------------------------------
            [f, selectivity, p, act_peak, act_total] = delay_calc_selecivity(win_row, win_col, dist_mat, p, p.numInputDims(layer));
