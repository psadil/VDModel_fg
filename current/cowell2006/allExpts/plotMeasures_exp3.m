function [] = plotMeasures_exp3(firstRat, lastRat)
% analyze recognition
...
for rat = firstRat:lastRat
    for session = 1:p.nSess
        ...
        recogByLayer(rat,session,1,1:p.nMismatch,:) = p.recogByLayer(:,1,:);
        if p.layer == 1
            recog(rat,p.stimCond,1,1:p.nMismatch,:) = p.recognition;
        else
            recogByLayer(rat,session,2,1:p.nMismatch,:) = p.recogByLayer(:,2,:);
            recog(rat,p.stimCond,2,1:p.nMismatch,:) = p.recognition ;
        end
...
[recog_mean, recog_sem, ~, recogByLayer_mean, recogByLayer_sem, ...
...
rats = zeros(nRats,p.nSess/2,2);
for stimCond = 1:p.nSess/2
    rats(:,stimCond,:) = ...
        squeeze(mean(mean(measure(:,stimCond,:,1:p.nMismatch,:),4),5));
end
...
for sess = 1:p.nSess
    ...
    if sess <= p.nSess/2;
        rats_byLayer(:,sess,1) = squeeze(mean(mean(measureByLayer(:,sess,1,1:p.nMismatch,:),4),5));
    elseif sess > p.nSess/2;
        idx = sess - length(p.nTrials);
        rats_byLayer(:,sess,:) = squeeze(mean(mean(measureByLayer(:,sess,:,1:p.nMismatch,:),4),5));
    end
end
mn_byLayer = squeeze(mean(rats_byLayer,1));
sem_byLayer = squeeze(std(rats_byLayer,1) ./ sqrt(nRats));

matrixToSave = [rats_byLayer(:,1:sess/2,1); rats_byLayer(:,(sess/2)+1:sess,2)];
