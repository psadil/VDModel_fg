...
if p.sess == 1
    ...
    location = strcat(ROOT,'rats/rat', num2str(p.ratNum), '/W_stimCond', num2str(p.sess), '_layer', num2str(p.which_gp_layer), '.mat');
    save(location, 'weights');

else
    % if not first session, load rat
    location = strcat(ROOT,'rats/rat', num2str(p.ratNum), '/W_stimCond', num2str(p.sess-1), '_layer', num2str(p.which_gp_layer), '.mat');
...
%    load(location, 'weights');
%    
%    weights = (1 .* weights_pt) + (0 .* weights);
%    clear weights_pt 
% end
