...
for trial = 1:p.nTrials,
    ...
    stimuli = zeros(p.nMatch,p.components,2);
    if tType==1,
        %         tTypeStr='Mismatch';
        stimuli = p.stimuli_misMatch;
    elseif tType==2,
        %         tTypeStr='Match';
        stimuli = p.stimuli_match;
    end
...
%         inputs = cat(1,inputs, inputs+1, inputs+2);
        ...
        inp_mat = repmat(reshape(stimulus,[1 1 length(stimulus)]), [p.nRows p.nRows 1]);
        ...
        selectivity = zeros(p.numLayers,max(p.numGrids));
...
%                     winners(25,:) = [win_row-3 win_col];
                    ...
                    if cycle==1,  %need to compare last set of weights of old grid to
                        initial_selec(layer,grid) = selectivity_child;
                ...
                selectivity(layer,grid) = selectivity_child;
